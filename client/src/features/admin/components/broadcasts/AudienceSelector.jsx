  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-3 border rounded-lg text-left bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${isOpen ? 'border-blue-500 ring-2 ring-blue-100' : 'border-gray-300'
          }`}
      >
        <div className="flex flex-wrap gap-1 pr-8">
          {isAllSelected ? (
            <span className="text-gray-500">All {label.toLowerCase().replace('target ', '')}</span>
          ) : (
            selected.map((item) => (
              <span
                key={item}
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-auto">
          ...
          <div className="sticky top-0 bg-gray-50 px-4 py-2 border-b border-gray-100 flex justify-between items-center">
            <span className="text-xs text-gray-500 font-medium">{options.length} options</span>
            ...
            {options.map((option) => {
              ...
              return (
                <div
                  ...
                  className={`px-4 py-2 cursor-pointer flex items-center justify-between hover:bg-gray-50 transition-colors ${isSelected ? 'bg-blue-50/50' : ''
                    }`}
                >
